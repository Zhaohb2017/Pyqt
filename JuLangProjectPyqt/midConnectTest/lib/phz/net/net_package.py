...
class head:
    ...
    def encode(self):
        if self.cmd == 65534:
            ...
            format = '<%dshh' % len(self.flag)
        # if self.cmd == 1012:
        #     print("1012:", self.package_len, self.cmd, struct.pack(format, self.flag.encode(), self.package_len, self.cmd), len(struct.pack(format, self.flag.encode(), self.package_len, self.cmd)))
